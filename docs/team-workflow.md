...
│                         ↓                                   │
│  ┌── 前端 Track ────────────────────────────────────────┐   │
│  │  5a. DEVELOPER 前端 TDD 开发                          │   │
│  │      ├── 读 task + 验收标准                           │   │
│  │      ├── 🔴 写失败测试                                │   │
│  │      ├── 🟢 写最小实现                                │   │
│  │      └── ✅ 标记完成                                  │   │
│  │                       ↓                             │   │
│  │  6a. UI-TESTER Playwright 验证                       │   │
│  │      ├── 渲染检查（截图 + console）                    │   │
│  │      ├── 交互检查（点击/表单/导航）                    │   │
│  │      ├── 路由检查                                     │   │
│  │      └── Verdict: PASS → 下一个 / FAIL → 修复        │   │
│  │                       ↓                             │   │
│  │  7a. 循环 5a-6a 直到前端 task 完成                    │   │
│  └──────────────────────────────────────────────────────┘   │
│                         ↓                                   │
│  ┌── 后端 Track ────────────────────────────────────────┐   │
│  │  5b. DEVELOPER 后端 TDD 开发                          │   │
│  │      ├── 读 task + 验收标准                           │   │
│  │      ├── 🔴 写失败测试                                │   │
│  │      ├── 🟢 写最小实现                                │   │
│  │      └── ✅ 标记完成                                  │   │
...
```
┌──────────────┐   ┌──────────────┐   ┌──────────────┐   ┌──────────────────┐
│   security   │   │ cross-tester │   │  auto-test   │   │ opponent-prober  │
│  安全扫描     │   │  黑盒测试     │   │  全量测试     │   │  对手漏洞侦察     │
└──────────────┘   └──────────────┘   └──────────────┘   └──────────────────┘
       │                  │                  │                      │
       └──────── 汇总 ────┴──────────────────┴──────────────────────┘
                  │
...
2. **前端框架初始化** — 使用 `web-react-cli` skill 初始化前端框架
3. **共享代码** — 创建 shared 类型定义、工具函数、数据库 schema 等
4. **基础配置** — package.json、tsconfig、lint 配置、环境变量模板等
5. **向用户确认** — 展示骨架结构 + 功能清单，等待用户确认
6. **Docker 环境** — 复制 docker 模板，启动服务
7. **提交并推送** — push 到 main，确保骨架可运行
8. **任务分配** — 按依赖关系分配任务给 developer-1/2/3
...
第 1 个人（主控 Claude）
  ...
  ├── 🔵 用户确认 plan
  ├── scaffold: 后端 + 前端骨架 → 向用户确认 → push main → 分配任务
  ├── 退出 Claude
  └── 也参与开发（选剩余任务，如 #5,#6）
